...
const checkIfNoDataAvailable = (array) => {
    return array === null || array.length === 0 || (array.length === 1 && (!array[0].from || array[0].from === '-'));
};

const UserData = ({ user }) => {
    const { firstName, lastName, email, gender, jobPosition, dateOfBirth, workExperience, skills, languages, interests, education } = user;
                ...
                <div className={styles.section}>
                    ...
                    {workExperience.map((experience, index) => (
                        <div key={index}>
                            ...
                            <p>{experience.position} at {experience.place}</p>
                        </div>
                    ))}
                </div>
            )}
                ...
                <div className={styles.section}>
                    ...
                    {education.map((edu, index) => (
                        <div key={index}>
                            ...
                            <p>{edu.fieldOfStudy} at {edu.place}</p>
                        </div>
                    ))}
                </div>
            )}
